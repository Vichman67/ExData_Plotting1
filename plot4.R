...
# Page layout - 2x2 graphs
par(mfrow=c(2,2))
# Fix size of texts
par(ps=12)
# First plot creation
with(datos3, plot(Global_active_power ~ Timestamp, type = "l", xlab = "", ylab = "Global Active Power"))
# Second plot creation
with(datos3, plot(Voltage ~ Timestamp, type = "l", xlab = "datetime"))
# Third plot creation
...
legend("topright",  # Legend position
	...
	bty ="n")  # Supress legend outline
# Fourth plot creation
with(datos3, plot(Global_reactive_power ~ Timestamp, type = "l", xlab = "datetime"))
